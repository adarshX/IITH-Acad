# OR gate with noise
# N inputs
# L input nodes , M hidden nodes , 1 output node
...
import math
import random
N = 20  # no.of sample inputs

x0 = [[0,0],[0,1],[1,0],[1,1]]  #boolean input array
y = [0,1,1,1]                   #expected_output  array for boolean

x = x0
n1 = 6
n = N - n1 - 4
#noise =  [[0,0] for i in range(N)]
for i in range(n1):
	noise = np.random.uniform(0,0.2,2)  #adding noise term
	x.append(noise)


for i in range(n1):
	y.append(0)						#expected_output  array for all


for i in range(n):
	noise_1 = random.uniform(0,0.2)
	noise_2 = random.uniform(1,1.2)
	noise = (noise_1,noise_2)
	x.append(noise)


for i in range(n):
	y.append(1)						#expected_output  array for all

print('Input array')
print(x)
print('-------')
print('expected_output array')
print(y)
...
def backward():
	...
	for i in range(2):
		...
		wo_new[i] = wo[i] - p1
	#print(wo_new)
	t11 = [0 for i in range(N)]
	t12 = [0 for i in range(N)]
	t21 = [0 for i in range(N)]
	t22 = [0 for i in range(N)]
	for i in range (N):
		a1 = y[i] - y_p[i]
		a2 = der_sigmoid(sum2[i])		
		a31 = der_sigmoid(sum1[i][0])
		a32 = der_sigmoid(sum1[i][1])
		t11[i] =  2 * a1 * a2 * wo[0] * a31 * x[i][0]  * z[i][0]
		t12[i] =  2 * a1 * a2 * wo[0] * a31 * x[i][1]  * z[i][0]
		t21[i] =  2 * a1 * a2 * wo[1] * a32 * x[i][0]	* z[i][1]
		t22[i] =  2 * a1 * a2 * wo[1] * a32 * x[i][1]	* z[i][1]
